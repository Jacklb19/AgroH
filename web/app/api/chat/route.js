...
REGLA ABSOLUTA — DEBES seguirla sin excepción:
Ante CUALQUIER pregunta sobre municipios, cultivos, rendimientos, alertas, clima o producción agrícola, SIEMPRE debes llamar una herramienta PRIMERO. NUNCA respondas con conocimiento general cuando hay herramientas disponibles. Si no llamas una herramienta, tu respuesta es incorrecta.

Guía de herramientas (úsalas proactivamente):
...
Si la BD no tiene datos, dilo en lenguaje simple y explica por qué.`;
  ...
  try {
    switch (name) {
      ...
      case "buscar_prediccion": {
        ...
        `, params);
        return { predicciones: rows, total: rows.length };
      }
      ...
      case "buscar_clima": {
        ...
        `, [`%${args.municipio}%`]);
        return { registros_climaticos: rows, total: rows.length };
      }
